...
const VolcanoMarker: React.FC<DisasterMarkerProps> = ({ key, markerData, mapRef, google, ...props }) => {
  const [passesFilter, setPassesFilter] = useState(true);
  const [isMouseOver, setIsMouseOver] = useState(false);

  const dispatch = useDispatch();
  let filter = useSelector((state: RootState) => state.disasterFilters.volcanoes);
